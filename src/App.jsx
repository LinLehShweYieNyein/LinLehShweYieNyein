...
function App() {
  ...
  const handleSubmit = (e) => {
    ...
    const product = accessoryData.find(accessory => accessory.code === productCode);
    const order = {
      ...product,
  ...
  return (
    <>
          ...
          <Col xs={10}>
            <select ref={productRef} onChange={updatePrice}>
              {accessoryData.map((accessory, index) => (
                <option key={index} value={accessory.code}>
                  {accessory.name}
                </option>
              ))}
            </select>
      ...
      <Container>
        ...
        <span>Sort</span>
        <Button variant="link" onClick={deleteItemByIndex}>Clear
        </Button>
        <DataTable 
